...
type ClientManager interface {
	...
	// Clusters lists the clusters the 'api/v1/cluster/{cluster}' route prefix accepts, the local one first.
	Clusters(req *restful.Request) (*ClusterList, error)
	// RemoteConfig returns the rest config of the named remote cluster, parsed from its kubeconfig Secret,
	// which is read with the caller's credentials: not found when no such cluster is registered, forbidden
	// when the caller may not read the Secret.
	RemoteConfig(req *restful.Request, name string) (*rest.Config, error)
}
...
type ResourceVerber interface {
	...
	Delete(kind string, namespaceSet bool, namespace string, name string, propagation string) error
}
