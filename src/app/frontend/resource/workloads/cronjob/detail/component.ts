...
export class CronJobDetailComponent implements OnInit, OnDestroy {
  ...
  // signal + getter: the template still reads `cronJob`, but the read now
  ...
  private readonly cronJobSig = signal<CronJobDetail>(undefined);
  get cronJob(): CronJobDetail {
    return this.cronJobSig();
  }
