...
export class SecretDetailComponent implements OnInit, OnDestroy {
  ...
  // signal + getter: the template still reads `secret`, but the read now
  ...
  private readonly secretSig = signal<SecretDetail>(undefined);
  get secret(): SecretDetail {
    return this.secretSig();
  }
