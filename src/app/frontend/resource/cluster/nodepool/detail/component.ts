...
export class NodePoolDetailComponent implements OnInit, OnDestroy {
  ...
  // signal + getter: the template still reads `nodePool`, but the read now
  ...
  private readonly nodePoolSig = signal<NodePoolDetail>(undefined);
  get nodePool(): NodePoolDetail {
    return this.nodePoolSig();
  }
