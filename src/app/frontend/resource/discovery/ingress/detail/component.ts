...
export class IngressDetailComponent implements OnInit, OnDestroy {
  // signal + getter: the template still reads `ingress`, but the read now
  ...
  private readonly ingressSig = signal<IngressDetail>(undefined);
  get ingress(): IngressDetail {
    return this.ingressSig();
  }
